      ...
      <div className="flex justify-between items-end mb-8">
        ...
        <div className="flex items-center gap-3 p-4 bg-white border border-outline-variant rounded-xl">
          <div className="text-right">
            <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Global Risk Score</p>
          ...
          <div className="w-20 h-3 bg-slate-100 rounded-full overflow-hidden">
            <div className="h-full bg-primary rounded-full transition-all" style={{ width: `${overallRisk * 100}%` }} />
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-12 gap-6">
        ...
        <div className="col-span-8 space-y-6">
          <div className="bg-white border border-outline-variant rounded-xl p-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              {RISK_GATES.map(g => (
                <div key={g.label} className="p-4 bg-surface-container-low rounded-xl border border-outline-variant">
                  ...
                  <div className="w-full h-1.5 bg-slate-200 rounded-full">
                    <div className={`h-full ${g.barColor} rounded-full transition-all`} style={{ width: `${g.value}%` }} />
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="col-span-4 space-y-6">
          ...
          <div className="bg-white border border-outline-variant rounded-xl p-6">
            ...
            <div className={`flex items-center justify-between p-4 rounded-xl border-2 transition-colors ${agentPaused ? 'bg-error/5 border-error/30' : 'bg-tertiary/5 border-tertiary/20'}`}>
              <div>
              ...
              <button
                ...
                className={`relative inline-flex h-6 w-12 items-center rounded-full transition-colors ${agentPaused ? 'bg-error' : 'bg-tertiary'}`}
              >
          ...
          <div className="bg-white border border-outline-variant rounded-xl p-6 space-y-5">
            ...
            {[
              ...
              <label key={label} className="block">
                <div className="flex justify-between text-[11px] font-semibold text-outline uppercase tracking-wider mb-2">
                  <span>{label}</span>
                  <span className="text-on-surface">{fmt(val)}</span>
                </div>
                <input
            ...
            <button type="button" className="w-full bg-primary text-white py-3 rounded-xl font-manrope font-bold text-sm hover:opacity-90 transition-all">
              Save Parameters
            </button>
          </div>
        </div>
