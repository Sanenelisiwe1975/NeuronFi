/**
 * @file app/api/resolutions/route.ts
 * @description GET /api/resolutions — reads AI oracle resolution status for all markets.
 */
      ...
      addresses.map(async (addr) => {
        try {
          ...
          const [question, res] = await Promise.all([
            ...
            resolver.resolutions(marketId) as Promise<{ outcome: bigint; source: bigint; timestamp: bigint; resolvedBy: string; finalized: boolean }>,
          ]);
          ...
          const proposed = ts > 0;
          const disputeWindowEnds = ts > 0 ? new Date((ts + 86400) * 1000).toISOString() : null;
